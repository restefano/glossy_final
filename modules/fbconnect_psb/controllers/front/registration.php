...
class FBConnect_PSBRegistrationModuleFrontController extends ModuleFrontController
{
	...
	*/
	public function initContent()
	{
		...
		// current user state Logged In with Facebook
		$fb_user_isLogged = ($user) ? TRUE : FALSE;

		if ($fb_user_isLogged && Tools::getValue('done'))
		{
		...
		if(!$fb_user_isLogged)
		{
			// The str_replace function is to remove the "done=1" parameter from the return URL after login
			// this is in case the user is logged out of Facebook than submits the reg form. Is this over kill?
			$this->context->smarty->assign(array(
				'fbconnect_psb_loginURL' => str_replace('%3Fdone%3D1', '', $facebook->getLoginUrl())
			));
		}
		...
		$this->context->smarty->assign(array(
			...
			'fb_user_isLogged' => $fb_user_isLogged
		));
