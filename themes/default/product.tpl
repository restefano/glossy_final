	...
	<div id="pb-right-column">
		...
		{if $have_image}
			<span id="view_full_size">
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}"{if $jqZoomEnabled} class="jqzoom"{/if} title="Clique ou toque para ver ampliada"alt="{$product->name|escape:'htmlall':'UTF-8'}" id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}"/>
			...
			{if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
				...
				{else}
					<div class="price">
						...
						ou
						<br>
						<a title="Conectar-se usando sua conta no Facebook" class="button" style="background-color:#3B5998" href="{$link->getModuleLink('fbconnect_psb', 'registration', array(), true)}">Registre-se com o Facebook</a>
						<br><br>
						Já é cliente ?<br>
						<a title="{l s='Login with your Facebook Account' mod='fbconnect_psb'}" class="button_large"  style="background-color:#3B5998"  href="{$link->getModuleLink('fbconnect_psb', 'login', array(), true)}">Entre com o Facebook</a>
						<br><br>
					</div>
				{/if}
