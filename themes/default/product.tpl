...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<p class="warning_inline" id="last_quantities"{if (($product->quantity==0 AND $allow_oosp->value==0) OR ($product->quantity > $last_qties) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE)} style="display: none"{/if} style="color:red;">{l s='Warning: Last items in stock!'}</p>
